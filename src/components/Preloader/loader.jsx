...
export default function Loader({ loadingImages }) {
  ...
  useEffect(() => {
    ...
    // Animate images out (reverse order)
    tl.to(
      ".loading-image",
      {
        height: 0,
    ...
    tl.to([progressTextRef.current, containerRef.current.querySelector(".loading-info")], { opacity: 0, duration: 0.5 }, "+=0.5");
    // Collapse container
    tl.to(loadingImagesContainerRef.current, { height: 0, opacity: 0, duration: 0.5 }, "-=0.3");
  ...
  return (
    <div ref={containerRef} className="w-[400px] max-w-sm mx-auto flex flex-col items-center">
      ...
      <div className="loading-info w-full flex justify-between items-center">
        <div className="overflow-hidden">
          <TextAnimation animateOnScroll={false} delay={0.5}>
            <span className="text-gray-800 text-sm">Coffee Bar, Ceramics & Furniture</span>
          </TextAnimation>
        </div>
        <div className="overflow-hidden min-w-[3ch] flex justify-end">
          {!showCounter ? (
            <TextAnimation animateOnScroll={false} delay={0.5}>
              <span ref={progressTextRef} className="text-gray-500 text-sm">
                0%
              </span>
            </TextAnimation>
          ) : (
